...
module.exports = function momentHandlebarsExtension(hbs, opts) {
  ...
  hbs.registerHelper('ago', (value, hash) => {
    ...
    return moment(v).locale(opts.locale).fromNow();
  });

  // Date format short
  hbs.registerHelper('df', (value, hash) => {
    return moment(value).locale(opts.locale).format('L');
  });

  // Date format Long
  hbs.registerHelper('dfl', (value, hash) => {
    return moment(value).locale(opts.locale).format('LL');
  });

  // Time format
  hbs.registerHelper('tf', (value, hash) => {
    return moment(value).locale(opts.locale).format('LT');
  });

  // Date-Time format
  hbs.registerHelper('dtf', (value, hash) => {
    return moment(value).locale(opts.locale).format('LLL');
  });

  // Date-Time format with given format
  hbs.registerHelper('dff', ([value, format], hash) => {
    return moment(value).locale(opts.locale).format(format);
  });
